    ...
    QObject(parent)
{
    ...
    // drm_prime_out = std::make_unique<DRMPrimeOut>(3);
    // if (drm_prime_out->init("vc4") != 0) {
    //     qDebug() << "DRMPrimeOut init failed";
    // }
}
...
void AVCodecDecoder::on_new_frame(AVFrame *frame)
{
    ...
    // drm_prime_out->display_frame(frame);
    // QMetaObject::invokeMethod(videoItemPtr, "submitFrame",Qt::QueuedConnection, Q_ARG(AVFrame*, frame));
}
    ...
    av_log_set_level(AV_LOG_TRACE);
     ...
     if (decoder->id == AV_CODEC_ID_H264) {
         ...
         if(!stream_config.enable_software_video_decoder){
             auto tmp = avcodec_find_decoder_by_name("h264_v4l2m2m");
             if(tmp!=nullptr){
                 decoder = tmp;
                 use_pi_hw_decode=true;
                 wanted_hw_pix_fmt = AV_PIX_FMT_DRM_PRIME; // GPU-backed frame

                // Create hardware device context
                // if (av_hwdevice_ctx_create(&hw_device_ctx, AV_HWDEVICE_TYPE_V4L2,
                //                         nullptr, nullptr, 0) < 0) {
                //     qWarning() << "Failed to create V4L2 hw device context, fallback to software";
                //     use_pi_hw_decode = false;
     ...
     // Attach HW device context to decoder
    if (use_pi_hw_decode && hw_device_ctx) {
